...
class Glove:
    ...
    def get_context(self, i):
        ...
        for w in range(left_context, i):      
            if 0 <= w < len(self.tokens):
                context.append(self.get_word_id(self.tokens[w]))
                
        for w in range(i + 1, right_context + 1):      
            if 0 <= w < len(self.tokens):
                context.append(self.get_word_id(self.tokens[w]))
    ...
    def loss( self ) :
        ...
        for i in self.X:
            for j in self.X[i]:
                dot_product = np.dot(self.w_vector[i], self.w_tilde_vector[j])
                loss_sum += self.f(self.X[i][j]) * (dot_product - math.log(self.X[i][j]))**2
    ...
    def compute_gradient(self, i, j) :
        ...
        dot_product = np.dot(self.w_vector[i], self.w_tilde_vector[j])
        f_xij = self.f(self.X[i][j])
        wi_vector_grad = f_xij * (dot_product - math.log(self.X[i][j])) * self.w_tilde_vector[j]
        
        wj_tilde_vector_grad = f_xij * (dot_product - math.log(self.X[i][j])) * self.w_vector[i]
    ...
    def train( self ) :
        ...
        word_ids = list(self.X.keys())  # focus word ids
        scores = []
        total_count = 0
        
        for i in word_ids:      # for all focus words
            count_sum = sum(self.X[i].values())  # Sum over all context word (neighbors) counts
            scores.append(count_sum)
            total_count += count_sum
            
        probs = [score / total_count for score in scores]    # probability / weight that a word i should be chosen
        
        last_loss = float('inf')
        
        pbar = tqdm(total=int(1e8))      # progress bar
        
        while ( self.patience > 0 ) and iterations < 1e8:      
            i = random.choices(word_ids, weights=probs)[0]    
            ...
            if iterations%100000 == 0 :
                ...
                print(f"Train loss at iteration {iterations}: {train_loss}")
...
def main() :
    ...
    parser.add_argument('--file', '-f', type=str,  default='shakespeare.txt', help='The files used in the training.')
    parser.add_argument('--output', '-o', type=str, default='vectors_shakespeare.txt', help='The file where the vectors are stored.')
